...
const debug = require( 'debug' )( 'DeviceServiceController' );

export default class DeviceServiceController {

  static readonly _ID = "DeviceServiceController";
  ...
  static async register( request: Request,
    ...
    try {
      ...
      */
      ...
      result = {
                 StatusCode: 200, //Ok
                 Code: 'SUCCESS_REGISTER_CREATE',
                 Message: await I18NManager.translate( strLanguage, 'Success register the device' ),
                 Mark: '90B0976CEED0' + ( cluster.worker && cluster.worker.id ? "-" + cluster.worker.id : "" ),
                 LogId: null,
                 IsError: false,
                 Errors: [],
                 Warnings: [],
  ...
  static async unregister( request: Request,
                           transaction: any,
                           logger: any ): Promise<any> {
    ...
    try {
      ...
      result = {
                 StatusCode: 200, //Ok
                 Code: 'SUCCESS_REGISTER_DELETE',
                 Message: await I18NManager.translate( strLanguage, 'Success delete the device register' ),
                 Mark: '4C153A590958' + ( cluster.worker && cluster.worker.id ? "-" + cluster.worker.id : "" ),
                 LogId: null,
                 IsError: false,
                 Errors: [],
                 Warnings: [],
      ...
      sourcePosition.method = this.name + "." + this.unregister.name;

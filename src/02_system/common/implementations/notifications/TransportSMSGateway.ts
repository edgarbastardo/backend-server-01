...
export default class TransportSMSGateway {
  ...
  static async send( transportOptions: any,
    ...
    try {
      ...
      const messageList = CommonUtilities.trimArray( messageOptions.body.text.split( "@__Message_Split__@" ) );

      for ( const strTo of toList ) {

        for ( const strMessage of messageList  ) {
          ...
          params.append( "request.PhoneNumber", strTo.replace( "-", "" ) ); //"3057769594",
          params.append( "request.Message", strMessage ); //messageOptions.body.text );  //"Test message",

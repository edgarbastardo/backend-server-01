...
export default class BIZOriginService extends BaseService {

  static readonly _ID = "bizOriginService";

  static async createOrUpdate( createOrUpdateData: any,
                               bUpdate: boolean,
                               transaction: any,
                               logger: any ): Promise<BIZOrigin> {
    ...
    try {
      ...
      let bizOriginInDB = await BIZOrigin.findOne( options );
      ...
      const strMark = "174F303EC010" + ( cluster.worker && cluster.worker.id ? "-" + cluster.worker.id : "" );
  ...
  static async getById( strId: string,
    ...
    try {
      ...
      const strMark = "C863D7A63E6E" + ( cluster.worker && cluster.worker.id ? "-" + cluster.worker.id : "" );

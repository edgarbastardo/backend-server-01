  ...
  static getStatement( strDialect: string,
    ...
    if ( strDialect === "mysql" ) {
      ...
      if ( strName === "getLastTicketImages" ) {
        ...
        strResult = SqlString.format( `Select ${params.SelectFields} From ticket_images As A Inner Join orders As B On B.id = A.order_id Where A.migrated = 0 And ( A.lock Is Null Or TIMESTAMP( A.lock ) <= DATE_SUB( ?, INTERVAL 5 MINUTE ) ) Order By A.created_at Desc Limit 3`, strNow );
